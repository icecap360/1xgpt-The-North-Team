...
class RawTokenDataset(TorchDataset):
    ...
    def __init__(
        ...
        for rank in tqdm(range(self.num_ranks)):
            ...
            new_act_list = []
            new_segment_list = []
            num_chunks = len(vid) - 2  # Because we access i and i+2
            for i in range(num_chunks):
                idx1 = i * window_size
                idx2 = (i + 2) * window_size
                ...
                if segment_ids[idx1] == segment_ids[idx2]:
                    ...
                    new_vid_list.append(combined_latent)

                    if with_act:
                        combined_actions = np.concatenate([act[idx1:idx1 + window_size], act[idx1 + window_size:idx1 + 2 * window_size]], axis=0)
                        new_act_list.append(combined_actions)
    ...
    def __getitem__(self, idx):
        ...
        if self.with_act:
            actions = self.action_list[idx * 17 : (idx + 2) * 17]
